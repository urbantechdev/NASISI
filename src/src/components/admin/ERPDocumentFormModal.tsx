...
export const ERPDocumentFormModal: React.FC<ERPDocumentFormModalProps> = ({
  ...
  const [vehicleReg, setVehicleReg] = useState(editDocument?.vehicleRegistration || '');
  const [driverName, setDriverName] = useState(editDocument?.driverName || '');
  const [driverPhone, setDriverPhone] = useState(editDocument?.driverPhone || '');
  const [dispatchedBy, setDispatchedBy] = useState(editDocument?.dispatchedBy || '');

  const [mpesaRef, setMpesaRef] = useState(editDocument?.mpesaRef || '');
  const [bankRef, setBankRef] = useState(editDocument?.bankRef || '');
  const [paymentTerms, setPaymentTerms] = useState(
    editDocument?.paymentTerms || ''
  );
