...
export const AdminERPSidebar: React.FC<AdminERPSidebarProps> = ({
  ...
  const { documents, inventory, currentUser, adminUsers } = useERP();
  ...
  const navItems: {
    ...
    },
    {
      ...
      description: 'Accounts, Roles & Credentials',
      icon: ShieldCheck,
      ...
      <AnimatePresence>
        {isOpenMobile && (
          <motion.div
            ...
            className="fixed inset-0 z-30 bg-slate-950/70 backdrop-blur-xs lg:hidden"
          />
        )}
      </AnimatePresence>

      {/* Persistent Sidebar with Dark Navy Blue Theme (Full-height extending behind the headerbar) */}
      <aside
        id="admin-erp-sidebar"
        className={`fixed inset-y-0 left-0 z-25 w-64 sm:w-72 bg-[#041429] text-slate-200 flex flex-col border-r border-[#0d2342] shadow-2xl transition-transform duration-300 ease-in-out lg:translate-x-0 overflow-hidden ${
          isOpenMobile ? 'translate-x-0 z-35' : '-translate-x-full'
        }`}
      >
        {/* Subtle Ambient Radial Highlight & Shadow Smoke Plumes */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-slate-950/40 rounded-full blur-2xl pointer-events-none" />
        <div className="absolute top-36 right-0 w-64 h-64 bg-blue-600/15 rounded-full blur-3xl pointer-events-none animate-smoke-1" />
        ...
        {/* Navigation Menu: Top padded so menu items stay in place below the headerbar without moving */}
        <div className="relative flex-1 overflow-y-auto px-3 pt-[180px] sm:pt-[186px] pb-6 space-y-1.5 scrollbar-thin scrollbar-thumb-white/10">
